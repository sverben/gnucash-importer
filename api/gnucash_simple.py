...
def jobToDict(job):
    if job is None:
        ...
        simple_job = {}
        simple_job['name'] = job.GetName()
        simple_job['id'] = job.GetID()
        simple_job['guid'] = job.GetGUID().to_string()
        simple_job['active'] = job.GetActive()
        simple_job['reference'] = job.GetReference()
        simple_job['owner'] = vendorToDict(job.GetOwner())
