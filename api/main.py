...
def format_european_money(value):
    locale.setlocale(locale.LC_ALL, 'nl_NL')
    return locale.format_string("%.2f", value, grouping=True)

def format_european_number(value):
    locale.setlocale(locale.LC_ALL, 'nl_NL')
    return locale.format_string("%.f", value, grouping=True)

environment = Environment(loader=FileSystemLoader("templates/"))
environment.filters["format_european_money"] = format_european_money
environment.filters["format_european_number"] = format_european_number
...
def api_transactions():
    ...
    if request.method == 'POST':
        ...
        currency = str(request.json.get('currency', ''))
        description = str(request.json.get('description', ''))
        num = str(request.json.get('num', ''))
        date_posted = str(request.json.get('date_posted', ''))

        splitvalue1 = int(request.json.get('splitvalue1', ''))
        splitaccount1 = str(request.json.get('splitaccount1', ''))
        splitvalue2 = int(request.json.get('splitvalue2', ''))
        splitaccount2 = str(request.json.get('splitaccount2', ''))
...
def api_transaction(guid):
    ...
    if request.method == 'GET':
        ...
        splitguid1 = str(request.json.get('splitguid1', ''))
        splitvalue1 = int(request.json.get('splitvalue1', ''))
        splitaccount1 = str(request.json.get('splitaccount1', ''))
        splitguid2 = str(request.json.get('splitguid2', ''))
        splitvalue2 = int(request.json.get('splitvalue2', ''))
        splitaccount2 = str(request.json.get('splitaccount2', ''))
...
def api_bills():
    ...
    if request.method == 'GET':
        ...
        id = str(request.json.get('id', None))
        ...
        vendor_id = str(request.json.get('vendor_id', ''))
        currency = str(request.json.get('currency', ''))
        date_opened = str(request.json.get('date_opened', ''))
...
def api_bill(id):
    ...
    if request.method == 'GET':
        ...
        vendor_id = str(request.json.get('vendor_id', ''))
        currency = str(request.json.get('currency', ''))
        date_opened = request.json.get('date_opened', None)
        notes = str(request.json.get('notes', ''))
        posted = request.json.get('posted', None)
        posted_account_guid = str(request.json.get('posted_account_guid', ''))
        posted_date = request.json.get('posted_date', '')
        due_date = request.json.get('due_date', '')
        posted_memo = str(request.json.get('posted_memo', ''))
        posted_accumulatesplits = request.json.get('posted_accumulatesplits',
            '')
        posted_autopay = request.json.get('posted_autopay', '')
        ...
        posted_account_guid = str(request.json.get('posted_account_guid', ''))
        transfer_account_guid = str(request.json.get('transfer_account_guid',
            ''))
        payment_date = request.json.get('payment_date', '')
        num = str(request.json.get('num', ''))
        memo = str(request.json.get('posted_memo', ''))
        auto_pay = request.json.get('auto_pay', '')
...
def api_bill_entries(id):
    ...
    if bill is None:
        ...
        if request.method == 'GET':
            ...
            description = str(request.json.get('description', ''))
            account_guid = str(request.json.get('account_guid', ''))
...
def api_jobs():
    ...
    jobs = getJobs(session.book, customer)
...
def api_invoice(id):
    ...
    if request.method == 'GET':
        ...
        posted = request.json.get('posted', None)
        posted_account_guid = request.json.get('posted_account_guid', None)
        posted_date = request.json.get('posted_date', None)
        due_date = request.json.get('due_date', None)
        posted_accumulatesplits = request.json.get('posted_accumulatesplits',
            None)
        billing_id = request.json.get('billing_id', None)
        ...
        try:
            invoice = updateInvoice(session.book, id, posted, posted_account_guid, posted_date, due_date, posted_accumulatesplits, billing_id)
        except Error as error:
            return Response(json.dumps({'errors': [{'type' : error.type,
                'message': error.message, 'data': error.data}]}), status=400,
                mimetype='application/json')
...
@app.route('/invoices/<id>/render', methods=['GET'])
def api_invoice_render(id):
    invoice = getInvoice(session.book, id)
    template = environment.get_template('invoice.html')
    currency_symbol = get_currency_symbol(invoice['currency'])
    asset_dir = path.join(path.dirname(path.realpath(__file__)), "templates/static")
    ...
    if invoice is None:
        ...
        invoice = getInvoice(session.book, id)
        buffer = io.BytesIO()
        html = HTML(string=template.render(invoice, currency_symbol=currency_symbol), base_url=asset_dir)
        ...
        if request.method == 'GET':
            ...
            try:
                entry = addEntry(session.book, id, date, description,
                    account_guid, quantity, price, action)
            except Error as error:
                return Response(json.dumps({'errors': [{'type' : error.type,
                    'message': error.message, 'data': error.data}]}),
                    status=400, mimetype='application/json')
...
def api_customers():
    ...
    if request.method == 'GET':
        ...
        address_line_1 = str(request.json.get('address_line_1', ''))
        address_line_2 = str(request.json.get('address_line_2', ''))
        address_line_3 = str(request.json.get('address_line_3', ''))
        address_line_4 = str(request.json.get('address_line_4', ''))
        phone = str(request.json.get('phone', ''))
        fax = str(request.json.get('fax', ''))
...
def getJobs(book, customer):
    query = gnucash.Query()
    query.search_for('gncJob')
    query.set_book(book)

    if customer is not None:
        query.add_guid_match(['owner', 'guid'], book.CustomerLookupByID(customer).GetGUID(), QOF_QUERY_AND)

    jobs = []
    for result in query.run():
        jobs.append(gnucash_simple.jobToDict(
            gnucash.gnucash_business.Job(instance=result)))
...
def addInvoice(book, id, customer_id, currency_mnumonic, date_opened, notes):
    ...
    try:
        date_opened = dateutil.parser.isoparse(date_opened)
    except ValueError:
        raise Error('InvalidDateOpened',
            'The date opened must be provided in ISO format',
            {'field': 'date_opened'})
...
def updateInvoice(book, id, posted, posted_account_guid, posted_date, due_date,
    posted_accumulatesplits, billing_id):
    ...
    if posted_date == '':
        ...
        try:
            posted_date = dateutil.parser.isoparse(posted_date)
        except ValueError:
            raise Error('InvalidDatePosted',
                'The date posted must be provided in ISO format',
                {'field': 'posted_date'})
    ...
    invoice.SetBillingID(billing_id)
...
def addEntry(book, invoice_id, date, description, account_guid, quantity, price, action):
    ...
    entry.SetInvTaxable(customer.GetTaxTable() is not None)
    entry.SetInvTaxIncluded(customer.GetTaxIncluded() == 1)
    entry.SetInvTaxTable(customer.GetTaxTable())
